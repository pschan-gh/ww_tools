...
$(document).ready(function() {
	...
	/* generate the MQEditor */
	var medtor = new MQEditor(input,button,$(input).parent('.input-append'));
	medtor.initialize();

	/* set medtor behavior specific to problem inputs */
	/* have button close other open medtors */
	medtor.button.click(function () {
	    var current = this;
	...
	/* have the medtor close if the input loses focus */
	...
	/* generate the MQEditor */
	var medtor = new MQEditor(input,button,'body',options);
	medtor.initialize();

    });
});

function MQEditor(field,button,container,userOptions) {
    ...
    if (typeof MQEditor.viewerCount == 'undefined' ) {
	MQEditor.viewerCount = 0;
    }
    MQEditor.viewerCount++;
    var viewerIndex = MQEditor.viewerCount;
    var me = this;
    ...
    popupdiv.css('width', '95%');
    popupdiv.css('padding-right', '1em');
	...
	popupttl.append($('<div>', {class : 'navbar-inner'})
            ...
            .append($('<div class="brand">MathQuill</div>')
	...
	*/
    ...
    popupdiv.append($('<div>', {class : 'well well-small medtorouter'}).text('MathQuill Interactive Previewer:  ').css('width','100%').append($('<a target="_blank" style="float:right" href="http://webwork.maa.org/wiki/Available_Functions">Available Functions</a>')).append($('<br/>')).append($('<span>', {id : 'math-field'+viewerIndex, class: 'math-field'}).css('width','100%')));
	...
	$('#medtor'+viewerIndex).html(me.inputTextBox.val());
    ...
    this.button.popover({html : 'true',
			     ...
			     // title : this.popoverTitle,
			     container : container
			}).closest("a.btn").on("click", function() {
                var $mqSpan = $(this).parent().find('span.math-field').first();

                console.log($mqSpan.attr('id'));
                var id = $mqSpan.attr('id');
                mqSpan = document.getElementById(id);

                var mathField = MQ.MathField(mqSpan, {
                    ...
                    handlers: {
                        edit: function() { // useful event handlers
                            ...
                            // $('input.codeshard').val(MQtoAM(mathField.latex()));
                            $(mqSpan).closest('.mv-container').find('input').val(MQtoAM(mathField.latex())); // simple API
                        }
                    }
                });

                var latex = AMtoMQ($(this).parent().find('input.codeshard').val());
	...
	if (me.renderingMode == "LATEX") {
	    $('#medtor'+viewerIndex).html('\\(' + text + '\\)');
	    MathJax.Hub.Queue([ "Typeset", MathJax.Hub, "medtor"+viewerIndex ]);
	} else if (me.renderingMode == "PGML") {
	    $('#medtor'+viewerIndex).html("`" + text + "`");
	    MathJax.Hub.Queue([ "Typeset", MathJax.Hub, "medtor"+viewerIndex ]);
	} else
